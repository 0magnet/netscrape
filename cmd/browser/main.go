...
// Command browser is netscrape: a web browser written in Go/wasm.
// Its chrome (tab strip, address bar, back/forward, reload) is DOM built from
...
//
// A host mounts it into an element (globalThis.__netscrapeMount, or the
// $NETSCRAPE_MOUNT — so `run browser.wasm` opens a browser in its own window.
//
